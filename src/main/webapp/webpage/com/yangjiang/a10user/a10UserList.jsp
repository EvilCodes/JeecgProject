...
<div class="easyui-layout" fit="true">
  ...
  <t:datagrid name="a10UserList" checkbox="true" fitColumns="false" title="大系统用户信息" actionUrl="a10UserController.do?datagrid" idField="id" fit="true" queryMode="group">
   ...
   <t:dgDelOpt title="删除" url="a10UserController.do?doDel&id={id}" />
   <t:dgToolBar title="录入" icon="icon-add" url="a10UserController.do?goAdd" funname="add"></t:dgToolBar>
   <t:dgToolBar title="编辑" icon="icon-edit" url="a10UserController.do?goUpdate" funname="update"></t:dgToolBar>
   <t:dgToolBar title="批量删除"  icon="icon-remove" url="a10UserController.do?doBatchDel" funname="deleteALLSelect"></t:dgToolBar>
   <t:dgToolBar title="查看" icon="icon-search" url="a10UserController.do?goUpdate" funname="detail"></t:dgToolBar>
 ...
 <script src = "webpage/com/yangjiang/a10User/a10UserList.js"></script>		
 <script type="text/javascript">
 $(document).ready(function(){
 		//给时间控件加上样式
 			$("#a10UserListtb").find("input[name='registertime_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#a10UserListtb").find("input[name='registertime_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#a10UserListtb").find("input[name='lastlogintime_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#a10UserListtb").find("input[name='lastlogintime_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#a10UserListtb").find("input[name='createtime_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#a10UserListtb").find("input[name='createtime_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#a10UserListtb").find("input[name='modifytime_begin']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 			$("#a10UserListtb").find("input[name='modifytime_end']").attr("class","Wdate").click(function(){WdatePicker({dateFmt:'yyyy-MM-dd'});});
 });
...
//导入
function ImportXls() {
	openuploadwin('Excel导入', 'a10UserController.do?upload', "a10UserList");
}

//导出
function ExportXls() {
	JeecgExcelExport("a10UserController.do?exportXls","a10UserList");
}

//模板下载
function ExportXlsByT() {
	JeecgExcelExport("a10UserController.do?exportXlsByT","a10UserList");
}
 </script>